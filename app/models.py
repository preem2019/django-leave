...
class Employee(models.Model):
    ...
    position = models.ForeignKey(
        Position, on_delete=models.CASCADE, db_column="position_id"
    )
    role = models.ForeignKey(Role, on_delete=models.CASCADE, db_column="role_id")
    must_change_password = models.BooleanField(
        default=True, verbose_name="ต้องเปลี่ยนรหัสผ่าน"
    )

    delegate_start_date = models.DateField(
        verbose_name="วันที่เริ่มมอบอำนาจ", null=True, blank=True
    )
    delegate_end_date = models.DateField(
        verbose_name="วันที่สิ้นสุดการมอบอำนาจ", null=True, blank=True
    )
    ...
    delegate_approver = models.ForeignKey(
        ...
        verbose_name="ผู้รับมอบอำนาจแทน",
    )
...
class LeaveRequest(models.Model):
    ...
    leave_duration = models.CharField(
        max_length=10, choices=DURATION_CHOICES, default="3 ชั่วโมง"
    )
...
class ApprovalHistory(models.Model):
    ...
    request = models.ForeignKey(
        LeaveRequest, on_delete=models.CASCADE, db_column="request_id"
    )
    approver = models.ForeignKey(
        Employee, on_delete=models.CASCADE, db_column="approver_id"
    )
...
class InOutHistory(models.Model):
    STATUS_CHOICES = [("OUT", "อยู่ข้างนอก"), ("COMPLETED", "กลับมาแล้ว")]
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name="in_out_histories"
    )
    ...
    status = models.CharField(
        max_length=10, choices=STATUS_CHOICES, default="OUT", null=True
    )
...
class VisitorLog(models.Model):
    ...
    contact_person = models.CharField(
        max_length=255, verbose_name="ติดต่อใคร (พนักงาน/แผนก)"
    )
    ...
    status = models.CharField(
        max_length=10, default="IN", choices=[("IN", "อยู่ในพื้นที่"), ("OUT", "ออกไปแล้ว")]
    )
